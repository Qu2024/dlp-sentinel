...
暂停只会断开浏览器和实时流服务的连接，不会清空本轮历史；暂停后按钮会变成“继续”，再次点击会沿用同一个实时 run，继续累计事件、会话、风险候选和总览指标。如果需要从头开始，点击“重置”。
...
前端主界面不再放“数据集/演示模式”下拉框，默认就是实时生成展示。右上角只显示当前运行模式和日期，避免演示时暴露内部测试数据选择。

展示流程建议：

- `实时演示`：点击“启动”，实时生成数据并同步处理风险；暂停后点“继续”接着跑；
- `总览`：查看本轮实时流累计指标和最新风险详情；
- `智能体`：查看规则识别、行为理解、评分、研判处置链路；
- `自适应规则`：查看生效规则和建议规则池；机器挖掘建议可以点击“加入规则”提升为生效场景规则，也可以点击“拒绝/弃用”移出建议池。

历史复盘样例仍保留给开发测试，可通过 URL 参数访问，例如 `http://127.0.0.1:5173/frontend/?mode=llm_report`。
...
4. 切到“智能体”，展示规则识别、行为理解、风险评分、研判处置的协同链路。
