...
class MultiAgentOrchestrator:
    ...
    def run(self, csv_path: str):
        workflow_trace = []
        agent_trace = []
        run_id = adaptive_rule_engine.generate_run_id()

        adaptive_prepare = adaptive_rule_engine.prepare_runtime(output_dir=self.output_dir)
        workflow_trace.append({
            "step": "自适应规则准备模块",
            "input": {"output_dir": str(self.output_dir)},
            "output": adaptive_prepare,
        })
        ...
        feedback_items = save_feedback_records(reports, self.output_dir, run_id=run_id)
        workflow_trace.append({
            "step": "反馈记录模块",
            "input": {"report_count": len(reports), "run_id": run_id},
            "output": {"feedback_items": len(feedback_items), "feedback_file": str(self.output_dir / "feedback.json")},
        })

        adaptive_update = adaptive_rule_engine.update_after_run(
            run_id=run_id,
            source_csv=csv_path,
